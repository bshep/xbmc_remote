...
if ( !$url ) {
  ...
  curl_setopt( $ch, CURLOPT_USERAGENT, $_REQUEST['user_agent'] ? $_REQUEST['user_agent'] : $_SERVER['HTTP_USER_AGENT'] );
  
  $postdata = str_replace("\\","",$_REQUEST['data']);
  // print $postdata;
...
if ( $_REQUEST['mode'] == 'native' ) {
  ...
  // Get JSONP callback.
  $jsonp_callback = $enable_jsonp && isset($_REQUEST['callback']) ? $_REQUEST['callback'] : null;
